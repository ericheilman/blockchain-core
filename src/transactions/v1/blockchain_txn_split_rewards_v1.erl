...
new(Gateway, Seller, Buyer, BuyerNonce, Percentage) when is_integer(Percentage)
                                        andalso Percentage > 0
                                        andalso Percentage =< 100 ->
    new(Gateway, Seller, Buyer, BuyerNonce, Percentage, 0).
...
new(Gateway, Seller, Buyer,  BuyerNonce, Percentage, AmountToSeller) when is_integer(AmountToSeller)
...
seller_has_percentage(#blockchain_txn_split_rewards_v1_pb{gateway=GatewayAddress,
    ...
    case blockchain_ledger_v1:find_gateway_info(GatewayAddress,Ledger) of
        {error, _} ->
            false;
        {ok, Gateway} ->
            OwnerPercentage = blockchain_ledger_gateway_v2:get_split(Gateway,Seller),
...
is_valid(#blockchain_txn_split_rewards_v1_pb{seller=Seller,
    ...
    Conditions = [{fun() -> Seller /= Buyer end, {error, seller_is_buyer}},
                  ...
                  {fun() -> is_valid_split_total(Txn, Ledger) end,
